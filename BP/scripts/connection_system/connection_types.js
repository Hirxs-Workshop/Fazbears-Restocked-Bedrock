...
export const LIGHT_BLOCK_CONFIGS = {
    ...
    "fr:pirate_cove_light_yellow": {
        alias: "Pirate cove light yellow",
        icon: "textures/fr_ui/light_test_icon",
        vfxEntity: "fr:pirate_cove_light_yellow_entity",
        spawnOffset: { x: 0, y: 0.4, z: 0 },
        destroyRadius: 1.5,
        powerState: "fr:lit",
        requiresCardinalRotation: true,
...
export function getLightConfig(blockId) {
    ...
    if (externalConfig && (externalConfig.type === "light" || externalConfig.type === "light_button")) {
        return {
            ...
            destroyRadius: 0.5,
            powerState: "fr:lit",
            icon: "textures/fr_ui/lightbulb_icon",
            ...externalConfig,
...
export function getSwitchConfig(blockId) {
    ...
    if (externalConfig && externalConfig.type === "switch") {
        return {
            icon: "textures/fr_ui/toggle_icon",
            powerState: "fr:switch_type",
            ...externalConfig,
            alias: externalConfig.alias || blockId
        };
...
export function spawnLightVfx(dimension, lightBlock, lightData, vfxCache) {
    ...
    if (config.useCommand && config.requiresFaceRotation) {
        ...
        try {
            ...
            console.warn(`[FR] Failed to summon ${config.vfxEntity}: ${e}`);
            return null;
        }
        ...
        try {
            ...
            if (spawnedEntities.length > 0) {
                spawnedEntity = spawnedEntities[0];
                
                if (config.colorState) {
                    const blockColor = lightBlock.permutation.getState(config.colorState) ?? 4;
                    const colorComponent = spawnedEntity.getComponent("minecraft:color");
                    if (colorComponent) {
                        colorComponent.value = blockColor;
                    }
                }
            }
        } catch (e) {
            console.warn(`[FR] Failed to get spawned ${config.vfxEntity}: ${e}`);
        }
        
        const isStageSpotlight = config.vfxEntity === "fr:stage_spotlight_vfx";
        vfxCache[key] = { 
            vfxType: config.vfxEntity,
            entity: spawnedEntity,
            isStageSpotlight: isStageSpotlight
        };
        return { key, vfxType: config.vfxEntity, entity: spawnedEntity, isStageSpotlight };
    }
...
export function getAllVfxEntities() {
    const vfxEntities = new Set();

    for (const key in LIGHT_BLOCK_CONFIGS) {
        if (LIGHT_BLOCK_CONFIGS[key].vfxEntity) {
            vfxEntities.add(LIGHT_BLOCK_CONFIGS[key].vfxEntity);
        }
    }
    ...
    for (const config of externalTypes) {
        if (config.vfxEntity) {
            vfxEntities.add(config.vfxEntity);
        }
    }

    vfxEntities.add("fr:hallway_lamp_vfx");

    return Array.from(vfxEntities);
}
