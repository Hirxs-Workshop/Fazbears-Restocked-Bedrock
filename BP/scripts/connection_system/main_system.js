...
import { dynamicToast, dynamicToastEvent, cleanupLampVfxEntitiesOnReload, cleanupLampVfxEntitiesSilent, getLinePoints, turnOffLight, lampVfxEntities, getExistingVfxEntity, rebuildVfxCache, findAnyVfxEntityAtLocation } from "./utils.js";
...
import { isLightBroken, repairLight } from "./broken_lights.js";
    ...
    system.runTimeout(() => {
        ...
        for (const conn of connections) {
            try {
                ...
                if (lightBlock && lightBlock.typeId === "fr:stage_spotlight") {
                    ...
                    const existingSpotlights = dimension.getEntities({
                    ...
                    if (existingSpotlights.length > 0) {
                        lampVfxEntities[key] = {
                            vfxType: "fr:stage_spotlight_vfx",
                            entity: existingSpotlights[0],
                            isStageSpotlight: true
                        };
                    }
                }
            } catch { }
...
world.afterEvents.playerInteractWithBlock.subscribe(event => {
    ...
    if (item && (item.typeId === "fr:wrench" || item.typeId === "fr:faz-diver_security" || item.typeId === "fr:faz-diver_repairman")) {
        ...
        if (category === "switch" || category === "generator") {
            ...
            if (category === "generator") {
                ...
                if (!gen) {
                    const genConfig = FRAPI.getGeneratorConfig(block.typeId);
                    const capacity = genConfig ? (genConfig.capacity ?? MAX_ENERGY) : MAX_ENERGY;
                    ...
                    gen = {
                        ...
                        active: true,
                        radius: 32,
                        customConfig: genConfig ? {
                            consumePerTick: genConfig.consumePerTick,
                            consumePercentage: genConfig.consumePercentage,
                            linksCapacity: genConfig.linksCapacity
                        } : null
                    };
                    generators.push(gen);
                    setGenerators(generators);
        ...
        if (category === "light") {
            ...
            if (isLightBroken(block)) {
                repairLight(block, player);
                return;
            }
            ...
            if (selections[player.id] && (selections[player.id].category === "switch" || selections[player.id].category === "generator")) {
                ...
                let limit = isGenerator ? getGeneratorLimit() : getSwitchLimit();

                if (isGenerator) {
                    const genData = getGeneratorAt(source);
                    if (genData && genData.linksCapacity !== undefined) {
                        limit = genData.linksCapacity;
                    }
                }
                let connections = getConnections();
        ...
        try {
            ...
            if (switchBlock && lightBlock && isLightType(lightBlock.typeId)) {
                ...
                if (isActive) {
                    if (!lampVfxEntities[key]) {
                        const dimension = world.getDimension(conn.light.dimensionId);
                        const existingVfx = findAnyVfxEntityAtLocation(dimension, { x: conn.light.x + 0.5, y: conn.light.y + 0.5, z: conn.light.z + 0.5 });
                        if (existingVfx) {
                            const isStageSpotlight = existingVfx.vfxType === "fr:stage_spotlight_vfx";
                            lampVfxEntities[key] = {
                                vfxType: existingVfx.vfxType,
                                entity: existingVfx.entity,
                                isStageSpotlight: isStageSpotlight
                            };
                        } else {
    ...
    generators.forEach(gen => {
        if (gen.active && gen.energy > 0) {
            ...
            if (gen.infiniteEnergy) {
                gen.energy = gen.maxEnergy || MAX_ENERGY;
                return;
            }

            let consumption = (activeConsumers + BASE_CONSUMPTION) * (gen.consumptionRate || DEFAULT_CONSUMPTION_RATE) * CONSUMPTION_MULTIPLIER;

            if (gen.customConfig) {
                if (gen.customConfig.consumePerTick) {
                    consumption += gen.customConfig.consumePerTick;
                }
                if (gen.customConfig.consumePercentage) {
                    consumption += (gen.energy * (gen.customConfig.consumePercentage / 100));
                }
            }

            gen.accumulator += consumption;
...
function updateGeneratorTitle(player, generatorData) {
    const maxE = generatorData.maxEnergy || MAX_ENERGY;
    const energyPercentage = Math.floor((generatorData.energy / maxE) * 100);
    const dimension = world.getDimension(generatorData.pos.dimensionId);
