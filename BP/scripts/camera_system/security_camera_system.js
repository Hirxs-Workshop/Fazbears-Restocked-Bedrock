              ...
              if (player) {
                try {
                  ...
                  if (list.length > 0) {
                    ...
                    this.applyView(player, dimension, nextCam, session.pc, fromFazTab);
                  } else {
...
form.label(`dock:right_§lUser:\n§r${player?.nameTag ?? player?.name ?? "Player"}`); } catch {}
  ...
  applyView(player, dimension, camPosStr, pcPosStr, fromFazTab = false) {
    const pid = player.id;
    ...
    if (!camBlock || camBlock.typeId !== "fr:security_cameras") {
      ...
      if (camList.length > 0) {
        this.viewYaw.set(player.id, 0);
        system.run(() => this.applyView(player, dimension, camList[0], pcPosStr, fromFazTab));
      } else {
        try { player.runCommand(`camera @s clear`); } catch {}
        try { player.runCommand(`camera @s fade time 0 0 0`); } catch {}
        try { player.runCommand(`camera @s fov_reset`); } catch {}
    ...
    const finish = () => {
      ...
      this.viewSessions.set(player.id, { pc: pcPosStr, dim: dimension.id, cam: camPosStr, fromFazTab: fromFazTab === true });
        ...
        try {
          ...
          if (!camBlock || camBlock.typeId !== "fr:security_cameras") {
            ...
            if (camList.length > 0) {
              ...
              try { this.applyView(player, world.getDimension(sess.dim), nextCam, sess.pc, fromFazTab); } catch { this.applyView(player, dim, nextCam, sess.pc, fromFazTab); }
            } else {
    ...
    try {
      ...
      this.getWorldTimeLabelsAsync(player).then((labels) => {
        ...
        if (sel === 0) {
          ...
          const session = this.viewSessions.get(pid);
          const fromFazTab = session?.fromFazTab === true;
          this.applyView(player, dimension, camList[prevIdx], pcPosStr, fromFazTab);
        } else if (sel === 2) {
          if (!this.viewers.has(pid)) {
            try { player.runCommand(`camera @s clear`); } catch {}
            try { player.runCommand(`hud @s reset`); } catch {}
          ...
          this.applyView(player, dimension, camList[nextIdx], pcPosStr, fromFazTabNext);
        } else if (sel === 3) {
          this.toggleFlashlight(player, camPosStr, dimension);
          player.playSound("flashlight");
          system.run(() => this.applyControlOverlay(player, pcPosStr, camPosStr, dimension));
