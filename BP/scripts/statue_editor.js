...
function startRouteMarkingMode(player, routeId, entity) {
  ...
  player.sendMessage(
    ...
    dynamicToast(
      ...
      "§7Route marking mode started! §6Right-click§7 on blocks to place route points.",
  ...
  player.sendMessage(
    ...
    dynamicToast(
      ...
      "§7§6Sneak + Right-click§7 to finish and exit.",
...
export function handleRoutePointPlacement(player, block, blockFace) {
  ...
  if (result.success) {
    ...
    player.sendMessage(
      `§7Point §e#${nextOrder + 1}§7 placed at §8(${Math.floor(targetLoc.x)}, ${Math.floor(targetLoc.y)}, ${Math.floor(targetLoc.z)})`,
    );
    player.playSound("random.orb", { pitch: 1.2, volume: 0.5 });
...
export function stopRouteMarkingMode(player) {
  ...
  player.sendMessage(
    ...
    dynamicToast(
      ...
      `§7Route marking complete! Created §e${session.pointsCreated}§7 route points.`,
    ...
    if (actionIdx >= 0 && actionIdx < buttonActions.length) {
      ...
      switch (actionData.action) {
        ...
        case "select_pose":
          ...
          player.playSound("random.orb", { pitch: 1.2, volume: 0.5 });
          system.run(() =>
            showRoutePointConfigMenu(player, routePointData, "poses"),
          );
          break;
        case "select_variant":
          updateRoutePoint(
          ...
          player.playSound("random.orb", { pitch: 1.2, volume: 0.5 });
          system.run(() =>
            showRoutePointConfigMenu(player, routePointData, "variants"),
          );
          break;
