...
import { startPathfinding, stopPathfinding } from "./astar_pathfinding.js";
import {
    getAILevel,
    getAILevelStats,
    selectNightWaypoint,
...
export function linkWaypointToStatue(
    statueId,
    waypointLocation,
    dimensionId,
    order = 0,
    ...
    console.log(
        `[Pathing] Linked waypoint to statue ${statueId} . Total: ${waypointRegistry.get(statueId)?.length || 0} `);
}
...
export function getNightMoveTime(entity, nightNumber) {
    ...
    return config[nightNumber] || {
        moveTime: 0, enabled: false
    };
}
...
export function startBlockSelectorMode(player, statueId, entityName) {
    ...
    try {
        ...
        player.sendMessage(
            `§a[Pathing] Block selector mode activated for ${entityName} `);
...
export function updateSelectorPosition(player) {
    ...
    try {
        for (const entity of dimension.getEntities({
            type: "fr:terminal_animatronic_selector",
        }
        )) {
...
export function createWaypoint(location, dimensionId, statueId, options = {}
    ...
    if (waypointExistsAtPosition(location, dimensionId, statueId)) {
        return {
            error: "duplicate",
            message: "A waypoint already exists at this position",
        };
    }
    const existingWaypoints = getWaypointsForStatue(statueId);
    if (existingWaypoints.length >= MAX_WAYPOINTS_PER_STATUE) {
        return {
            error: "max_reached",
            message: `Maximum waypoints (${MAX_WAYPOINTS_PER_STATUE} ) reached`,
        };
    }
...
export function placeWaypointAtSelector(player) {
    ...
    if (result.error) {
        if (result.error === "duplicate") {
            player.sendMessage(
                `§c[Pathing] A waypoint already exists at this position!`);
    ...
    player.sendMessage(
        `§a[Pathing] Waypoint #${result.config.order} placed at (${Math.floor(location.x)} , ${Math.floor(location.y)} , ${Math.floor(location.z)} )`);
    return result;
}
...
export function isLinkableEntity(entity) {
    return (
        STATUE_TYPES.includes(entity.typeId) ||
        ANIMATRONIC_TYPES.includes(entity.typeId)
    );
}
...
export function spawnLureAtLocation(dimension, location) {
    try {
        ...
        console.log(
            `[Pathing] Spawned lure at ${Math.floor(location.x)} , ${Math.floor(location.y)} , ${Math.floor(location.z)} `);
    } catch (e) {
    ...
    try {
        if (typeof player.getGameMode === "function") {
            ...
            return (
                modeStr === "survival" ||
                modeStr === "adventure" ||
                modeStr.includes("survival") ||
                modeStr.includes("adventure")
            );
        }
...
export function detectNearbyPlayer(entity, dimension) {
    try {
        ...
        for (const player of allPlayers) {
            ...
            const effectiveRange = isSneaking
                ? DETECTION_CONFIG.SNEAK_RANGE
                : DETECTION_CONFIG.BASE_RANGE;
            if (distance > effectiveRange) continue;
            if (
                isSneaking &&
                Math.random() < DETECTION_CONFIG.SNEAK_CHANCE_REDUCTION
            ) {
                continue;
            }
            ...
            const angleToPlayer =
                Math.atan2(playerLoc.x - entityLoc.x, playerLoc.z - entityLoc.z) *
                (180 / Math.PI);
...
export function createPathingSession(
    entityId,
    statueId,
    entityType,
    dimensionId,
    ...
    console.log(
        `[Pathing] Created session ${sessionId} for entity ${entityId} (statue ${statueId} , starting wp ${randomIndex} )`);
    return sessionId;
}
    ...
    if (tempData) {
        try {
            const dimension = world.getDimension(
                tempData.dimensionId.replace("minecraft:", ""));
            for (const e of dimension.getEntities({
                type: tempData.statueType
            }
            )) {
        ...
        if (waypoints && waypoints[session.waypointIndex]) {
            try {
                const dim = world.getDimension(
                    session.dimensionId?.replace("minecraft:", "") || "overworld");
...
function getWaitTimeFromState(state) {
    switch (state) {
        case 0:
            return (
                MIN_WAIT_TIME + Math.random() * (DEFAULT_WAIT_TIME - MIN_WAIT_TIME)
            );
        case 1:
            return 600;
        case 2:
...
function selectNextWaypoint(pathData, waypoints, entityId = null) {
    console.log(
        `[Pathing] Selecting next waypoint. Total: ${waypoints.length} , Current: ${pathData.waypointIndex} `);
    pathData.failedWaypoints.delete(pathData.waypointIndex);
    const isNight = isNightTime();
    const nightPathingState = entityId ? getNightPathingState(entityId) : null;
    const useNightRandom =
        isNight &&
        (nightPathingState?.useRandomOrder ??
            NIGHT_MODE_CONFIG.USE_RANDOM_WAYPOINTS);
    if (useNightRandom && waypoints.length > 1) {
        console.log(`[Pathing] Using NIGHT MODE random waypoint selection`);
        const nextIndex = selectNightWaypoint(
            entityId || "unknown",
            waypoints,
            pathData.waypointIndex);
        if (entityId) {
            markWaypointVisited(entityId, nextIndex);
        }
        console.log(`[Pathing] Night mode selected waypoint: ${nextIndex} `);
        return nextIndex;
    }
    if (pathData.useRandomOrder && waypoints.length > 1) {
        ...
        let attempts = 0;
        while (
            pathData.failedWaypoints.has(nextIndex) &&
            attempts < waypoints.length
        ) {
...
function getAdjustedMoveSpeed(entity) {
    if (!entity) return PATHING_CONFIG.MOVE_SPEED;
    try {
        const aiLevel = getAILevel(entity);
        const stats = getAILevelStats(aiLevel);
        return PATHING_CONFIG.MOVE_SPEED * stats.speedMultiplier;
    } catch {
        return PATHING_CONFIG.MOVE_SPEED;
    }
}
function getAdjustedWaitTime(entity, baseWaitTime) {
    if (!entity) return baseWaitTime;
    try {
        const aiLevel = getAILevel(entity);
        const stats = getAILevelStats(aiLevel);
        return Math.max(60, Math.floor(baseWaitTime * stats.waitTimeMultiplier));
    } catch {
        return baseWaitTime;
    }
}
function handleStuckEntity(
    pathData,
    entity,
    waypoints,
    currentWp,
    ...
    pathData.retryCount++;
    console.log(
        `[Pathing] Entity stuck (attempt ${pathData.retryCount} /${PATHING_CONFIG.MAX_RETRIES} )`);
    if (entity?.dimension) {
    ...
    for (const [sessionId, pathData] of activePathing) {
        ...
        if (pathData.currentEntityId) {
            const entity = findEntityById(
                pathData.currentEntityId,
                pathData.entityType,
                pathData.dimensionId);
            if (!entity) {
...
export function cleanupBuggedWaypoints() {
        ...
        try {
            ...
            try {
                ...
                if (!block || block.typeId !== "fr:waypoint") {
                    console.log(
                        `[Pathing] Removing orphaned waypoint data at ${x}, ${y}, ${z}`,
                    );
                    world.setDynamicProperty(key, undefined);
                    removedCount++;
    ...
    console.log(
        `[Pathing] Checked ${checkedCount} waypoints, removed ${removedCount} bugged entries`,
    );
    return removedCount;
}
...
export function startPathingSimulation(statue, player, maxWaypoints = 10) {
    try {
        ...
        if (!waypointStatueId) {
            player.sendMessage(
                "§c[Simulation] §7No waypoint ID assigned to this statue!");
            return false;
        }
        ...
        if (!waypoints || waypoints.length === 0) {
            player.sendMessage(
                "§c[Simulation] §7No waypoints configured for this statue!");
            return false;
        }
        ...
        if (!platformLocation) {
            player.sendMessage(
                "§c[Simulation] §7No platform linked! Link a platform first.");
            return false;
        }
        ...
        player.sendMessage(
            `§7Waypoints to visit: §e${Math.min(maxWaypoints, waypoints.length)} `);
        system.run(() => {
            system.run(() => {
                try {
                    const sessionId = createPathingSession(
                    ...
                    if (!sessionId) {
                        player.sendMessage(
                            "§c[Simulation] §7Failed to create pathing session!");
                        return;
                    }
                    ...
                    if (session) {
                        ...
                        if (waypoints[session.waypointIndex]) {
                            spawnLureAtLocation(
                                animatronic.dimension,
                                waypoints[session.waypointIndex].location);
                        }
                    }
                    player.sendMessage(
                        `§a[Simulation] §7Session §e${sessionId} §7 started!`);
                } catch (e) {
...
function recordSimulationResult(
    ...
    console.log(
        `[Simulation] Recorded waypoint ${waypointIndex} : ${success ? "SUCCESS" : "FAIL"} (${simData.waypointsVisited} /${simData.maxWaypoints} )`);
}
function checkSimulationComplete(sessionId, pathData) {
    ...
    if (simData.waypointsVisited >= simData.maxWaypoints) {
        console.log(
            `[Simulation] Complete! Visited ${simData.waypointsVisited} waypoints`);
        return true;
    }
    const waypoints = waypointRegistry.get(pathData.statueId);
    if (waypoints && pathData.failedWaypoints) {
        const allFailed = waypoints.every((_, idx) =>
            pathData.failedWaypoints.has(idx));
        if (allFailed) {
...
function endSimulation(sessionId, animatronic) {
    ...
    const successRate =
        totalVisited > 0 ? Math.round((successCount / totalVisited) * 100) : 0;
    const summaryLines = [
        `§6•••••••••••••••••••••••••••••••••••`,
    ...
    if (simData.failures.length > 0) {
        ...
        for (const fail of simData.failures) {
            ...
            summaryLines.push(
                `§7  WP#${fail.waypoint} : §c${fail.reason || "Stuck/Timeout"} §7(${timeStr} s)${posStr} `);
        }
    }
    summaryLines.push(`§6•••••••••••••••••••••••••••••••••••`);
    if (player) {
    ...
    if (animatronic) {
        ...
        try {
            animatronic.removeTag("fr:simulation_mode");
        } catch { }
            ...
            try {
                ...
                if (statueData.waypointStatueId > 0) {
                    statue.setDynamicProperty(
                        "fr:statue_id",
                        statueData.waypointStatueId);
                }
                statue.setDynamicProperty(
                    "fr:platform_location",
                    JSON.stringify(platformLoc));
                system.run(() => {
                    try {
                        statue.teleport(statue.location, {
                            rotation: {
                                x: 0, y: statueData.rotation
                            }
                            ,
                        }
                        );
                ...
                if (player) {
                    player.sendMessage(
                        `§a[Simulation] §7Returned to platform successfully!`);
                }
            } catch (e) {
...
export function startPathingForAnimatronic(
    animatronic,
    waypointStatueId,
    forceStart = false) {
    console.log(
        `[Pathing] startPathingForAnimatronic called: entity=${animatronic?.id} , waypointId=${waypointStatueId} , force=${forceStart} `);
    ...
    if (session) {
        const waypoints = waypointRegistry.get(waypointStatueId);
        console.log(
            `[Pathing] Session created: ${sessionId} , waypoints=${waypoints?.length || 0} , startIndex=${session.waypointIndex} `);
        if (waypoints && waypoints[session.waypointIndex]) {
            spawnLureAtLocation(
                animatronic.dimension,
                waypoints[session.waypointIndex].location);
    ...
    try {
        ...
        switch (response.selection) {
            case 0:
                if (waypointCount > 0) {
                    player.sendMessage(
                        `§a[Pathing] §7${entityName} has ${waypointCount} waypoints configured`);
                } else {
                    player.sendMessage(
                        `§c[Pathing] §7No waypoints configured. Use 'Create Waypoints' to add some.`);
                }
            ...
            case 1:
                if (waypointCount === 0) {
                    player.sendMessage(
                        "§c[Pathing] §7No waypoints to test! Create waypoints first.");
                    system.runTimeout(() => showPathingSettingsMenu(player, entity), 5);
                } else {
                    player.sendMessage(
                        `§a[Pathing] §7Starting pathing test for ${entityName} ...`);
...
async function showSetIdMenu(player, entity) {
    ...
    try {
        ...
        if (isNaN(newId) || newId < 0) {
            player.sendMessage(
                "§c[Pathing] §7Invalid ID. Please enter a positive number.");
        } else {
...
function adjustSelectorDistance(player, direction) {
    ...
    const newDist = Math.max(
        1,
        Math.min(MAX_SELECTOR_DISTANCE, currentDist + direction));
    const newPos = {
...
export async function showWaypointConfigMenu(player, waypointData) {
    ...
    try {
        ...
        switch (response.selection) {
            ...
            case 1:
                system.runTimeout(
                    () => showWaypointEditMenu(player, location, dimensionId, config),
                    1);
                break;
            case 2:
            ...
            case 3:
                cancelBlockSelectorMode(player);
                player.sendMessage(
                    `§a[Pathing] Waypoint placement complete. Total: ${getWaypointsForStatue(state.statueId).length} `);
                break;
        }
    } catch (e) {
...
export async function showWaypointEditMenu(
    player,
    location,
    dimensionId,
    currentConfig) {
    ...
    const currentWaitIndex = Math.min(
        7,
        Math.floor(currentConfig.waitTime / 600));
    const form = new ModalFormData()
...
export async function showWaypointAbilitiesMenu(
    player,
    location,
    dimensionId,
    currentConfig) {
    ...
    const hasBlackout = abilities.some(
        (a) => a.type === ABILITY_TYPES.CAMERA_BLACKOUT);
    ...
    const form = new ActionFormData()
        ...
        .body(
            `§7Configure special abilities for this waypoint.\n§7These trigger when the animatronic arrives.`)
    ...
    try {
        ...
        switch (response.selection) {
            ...
            case 1:
                system.runTimeout(
                    () =>
                        showEmitSoundConfig(player, location, dimensionId, currentConfig),
                    1);
                break;
            case 2:
...
async function showCameraBlackoutConfig(
    player,
    location,
    dimensionId,
    currentConfig) {
    const abilities = currentConfig.abilities || [];
    const existingBlackout = abilities.find(
        (a) => a.type === ABILITY_TYPES.CAMERA_BLACKOUT);
    const form = new ModalFormData()
    ...
    try {
        ...
        currentConfig.abilities = (currentConfig.abilities || []).filter(
            (a) => a.type !== ABILITY_TYPES.CAMERA_BLACKOUT);
        if (enabled) {
        ...
        system.runTimeout(
            () =>
                showWaypointAbilitiesMenu(player, location, dimensionId, currentConfig),
            1);
    } catch (e) {
        console.warn("[Pathing] Error in camera blackout config:", e);
    }
}
async function showEmitSoundConfig(
    player,
    location,
    dimensionId,
    currentConfig) {
    const abilities = currentConfig.abilities || [];
    const existingSound = abilities.find(
        (a) => a.type === ABILITY_TYPES.EMIT_SOUND);
    const soundOptions = [
        "ambient.cave",
        "mob.zombie.say",
        "random.door_open",
    ...
    const currentSoundIndex = existingSound
        ? soundOptions.indexOf(existingSound.soundId)
        : 0;
    const form = new ModalFormData()
        .title("§l§aEmit Sound")
    ...
    try {
        ...
        if (enabled) {
            ...
            player.sendMessage(
                `§a[Pathing] Sound enabled: ${soundOptions[soundIndex]} `);
        } else {
...
function executeCameraBlackout(ability, dimension) {
    ...
    for (const player of dimension.getPlayers()) {
        try {
            ...
            if (isViewingCamera) {
                if (ability.cameraId) {
                    const viewingCameraId = player.getDynamicProperty(
                        "fr:current_camera_id");
...
export async function showNightTimeConfigMenu(player, entity) {
    ...
    let bodyText =
        "§7Configure when the animatronic starts moving each night.\n\n";
    for (let night = 1;
        night <= MAX_NIGHTS;
        night++) {
    ...
    try {
        ...
        if (response.selection >= 0 && response.selection <= 6) {
            ...
            system.runTimeout(
                () => showNightTimeEditMenu(player, entity, nightNumber),
                1);
...
async function showNightTimeEditMenu(player, entity, nightNumber) {
    const config = loadNightTimeConfig(entity) || {};
    const nightConfig = config[nightNumber] || {
        moveTime: 0, enabled: false
    };
    const timeOptions = [
        "12:00 AM (Midnight)",
    ...
    try {
        ...
        if (enabled) {
            ...
            player.sendMessage(
                `§a[Pathing] Night ${nightNumber} set to start at ${timeStr} `);
        } else {
            config[nightNumber] = {
                moveTime: 0,
                enabled: false,
...
const VISUALIZATION_CONFIG = {
    ...
    NORMAL_PARTICLE_TYPE: "fr:raytest",
    LINE_PARTICLE_TYPE: "fr:raytest",
};
export function startWaypointVisualization(statueId, dimension, playerId) {
    ...
    if (waypoints.length === 0) {
        console.log(
            `[Visualization] No waypoints to visualize for statue ${statueId} `);
        return false;
    }
    ...
    console.log(
        `[Visualization] Started visualization for statue ${statueId} with ${waypoints.length} waypoints`);
    return true;
}
export function stopWaypointVisualization(statueId) {
    ...
    if (visualizationStates.size === 0 && visualizationIntervalId !== null) {
        ...
        console.log(
            `[Visualization] Cleared visualization interval - no active visualizations`);
    }
}
...
function processWaypointVisualization() {
    ...
    for (const [statueId, state] of visualizationStates) {
        ...
        try {
            ...
            console.warn(
                `[Visualization] Error processing visualization for statue ${statueId} :`,
                e);
        }
    }
}
function spawnWaypointMarkerParticles(
    dimension,
    location,
    isHighlighted,
    order) {
...
export function startRouteTest(entity, player) {
    ...
    try {
        ...
        system.runTimeout(() => {
            try {
                ...
                console.log("[Route Test] Disabled AI walk on spawned animatronic");
            } catch (e) {
                console.warn("[Route Test] Error disabling AI walk:", e);
            }
        }
            , 2);
        entity.remove();
    ...
    player.sendMessage(
        `§a[Route Test] §7Starting test with ${waypoints.length} waypoints...`);
    player.sendMessage(
        "§7Press §eSNEAK + INTERACT§7 on the animatronic to cancel.");
    const firstWaypoint = waypoints[0];
    spawnLureAtLocation(dimension, firstWaypoint.location);
    console.log(
        `[Route Test] Started session ${sessionId} for statue ${statueId} `);
    return sessionId;
}
export function cancelRouteTest(sessionId, reason = "User cancelled") {
    ...
    const animatronic = findEntityById(
        session.entityId,
        session.entityType,
        session.dimensionId);
    if (animatronic) {
...
function returnAnimatronicToPlatform(
    session,
    animatronic,
    reason = "Test complete") {
    if (session.state === "returning" || session.isReturning) return;
    ...
    notifyRouteTestPlayer(
        session,
        `§a[Route Test] §7${reason} . Visited ${visitedCount} /${totalCount} waypoints. Walking back to platform...`);
    try {
        animatronic.triggerEvent("fr:start_walking");
    } catch { }
    const returnSessionId = startPathfinding(animatronic, platformLoc, {
        pose: 0,
        variant: session.variantIndex,
        rotation: session.platformRotation || 0,
        waitTime: 0,
        detectPlayers: false,
        onArrival: (pathSession, ent) => {
            handleReturnToPlatformArrival(session, ent);
        }
        ,
        onFailed: (pathSession) => {
            handleReturnToPlatformFailed(session, animatronic);
        }
        ,
    }
    );
    if (!returnSessionId) {
        console.warn(
            "[Route Test] Could not start return pathfinding, using teleport fallback");
        finalizeReturnToPlatform(session, animatronic);
    } else {
        session.returnPathfindingSessionId = returnSessionId;
    }
}
function handleReturnToPlatformArrival(session, animatronic) {
    if (!session || !animatronic) return;
    console.log(`[Route Test] Arrived back at platform: ${session.sessionId} `);
    try {
        animatronic.triggerEvent("fr:stop_walking");
    } catch { }
    notifyRouteTestPlayer(session, `§a[Route Test] §7Arrived at platform.`);
    finalizeReturnToPlatform(session, animatronic);
}
function handleReturnToPlatformFailed(session, animatronic) {
    if (!session) return;
    console.warn(
        `[Route Test] Failed to walk back to platform: ${session.sessionId} `);
    notifyRouteTestPlayer(
        session,
        `§c[Route Test] §7Could not walk back, teleporting...`);
    finalizeReturnToPlatform(session, animatronic);
}
function finalizeReturnToPlatform(session, animatronic) {
    if (!session) return;
    const platformLoc = session.platformLocation;
    const dimension = animatronic?.dimension;
    if (!dimension) {
            ...
            try {
                ...
                system.run(() => {
                    try {
                        statue.teleport(statue.location, {
                            rotation: {
                                x: 0, y: session.platformRotation || 0
                            }
                            ,
                        }
                        );
                ...
                notifyRouteTestPlayer(
                    session,
                    `§a[Route Test] §7Converted back to statue at platform.`);
                routeTestSessions.delete(session.sessionId);
            } catch (e) {
...
function processRouteTestSession(session) {
    ...
    if (!animatronic) {
        console.log(
            `[Route Test] Animatronic not found for session ${session.sessionId} `);
        routeTestSessions.delete(session.sessionId);
        return;
    }
    ...
    switch (session.state) {
        case "moving":
            ...
            if (distance <= PATHING_CONFIG.ARRIVAL_DISTANCE) {
                ...
                console.log(
                    `[Route Test] Arrived at waypoint ${session.currentWaypointIndex + 1} /${waypoints.length} `);
                notifyRouteTestPlayer(
                    session,
                    `§a[Route Test] §7Waypoint ${session.currentWaypointIndex + 1} /${waypoints.length} reached`);
            } else if (stateElapsed > PATHING_CONFIG.STUCK_TIMEOUT_MS) {
                console.log(
                    `[Route Test] Stuck, teleporting to waypoint ${session.currentWaypointIndex} `);
...
function applyWaypointPose(animatronic, waypoint) {
    ...
    const variantIndex = waypoint.variant;
    try {
        animatronic.triggerEvent("fr:stop_walking");
    } catch { }
    try {
    ...
    if (variantIndex !== undefined && variantIndex !== null) {
        try {
            ...
            console.warn(`[Route Test] Failed to apply variant ${variantIndex} :`, e);
        }
    }
