...
const EXTERNAL_GENERATOR_CONFIGS = new Map();
...
export function registerConnectionType(config) {
    try {
        ...
        const validTypes = ["light", "light_button", "camera", "switch", "generator"];
        ...
        EXTERNAL_CONNECTION_TYPES.set(config.id, {
            ...
            spawnOffset: config.spawnOffset || null,
            destroyRadius: config.destroyRadius || null,
            powerState: config.powerState || null,
            requiredStates: config.requiredStates || [],
            cardinalState: config.cardinalState || null,
            offsetMap: config.offsetMap || null,
            rotationMap: config.rotationMap || null,
            hasVariants: config.hasVariants || false,
            variantState: config.variantState || null,
            variantOffsets: config.variantOffsets || null,
...
export function registerBlockVariants(blockId, variants) {
    try {
        ...
        const validColors = ["yellow", "gray", "blue", "green", "red", "purple", "cyan", "orange", "black", "white"];
...
export function registerGeneratorConfig(config) {
    try {
        if (!config || !config.blockId) {
            console.warn("[FR API] registerGeneratorConfig: Missing blockId");
            return false;
        }
        ...
        const generatorConfig = {
            ...
            productionRate: config.productionRate ?? 10,
            consumePerTick: config.consumePerTick,
            consumePercentage: config.consumePercentage,
            linksCapacity: config.linksCapacity,
            alias: config.alias || config.blockId,
            icon: config.icon || "textures/fr_ui/generator_icon"
        };

        EXTERNAL_GENERATOR_CONFIGS.set(config.blockId, generatorConfig);

        if (!EXTERNAL_CONNECTION_TYPES.has(config.blockId)) {
            const connectionType = {
                id: config.blockId,
                type: "generator",
                alias: generatorConfig.alias,
        ...
        console.error("[FR API] Error registering generator config:", error);
        return false;
    }
}
...
export function getGeneratorConfig(blockId) {
    return EXTERNAL_GENERATOR_CONFIGS.get(blockId) || null;
}
...
export function getAllGeneratorConfigs() {
    return new Map(EXTERNAL_GENERATOR_CONFIGS);
}
...
export function unregisterGeneratorConfig(blockId) {
    return EXTERNAL_GENERATOR_CONFIGS.delete(blockId);
}
...
export function clearAllRegistrations() {
    ...
    EXTERNAL_GENERATOR_CONFIGS.clear();
}
...
export function getRegistrationSummary() {
    return {
        ...
        generatorConfigs: EXTERNAL_GENERATOR_CONFIGS.size,
        details: {
            ...
            generatorConfigs: Array.from(EXTERNAL_GENERATOR_CONFIGS.keys())
        }
    };
}
...
function initializeScriptEventListeners() {
    ...
    system.afterEvents.scriptEventReceive.subscribe(event => {
        ...
        try {
            ...
            if (id === "fr_api:register_generator") {
                const data = JSON.parse(message);
                registerGeneratorConfig(data);
                return;
            }
