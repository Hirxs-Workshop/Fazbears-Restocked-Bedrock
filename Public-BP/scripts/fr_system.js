...
const animatronicLastPos = new Map();
const animatronicCentered = new Map();
const animatronicTeleportCooldown = new Map();
system.runInterval(() => {
    for (const dim of [world.getDimension('overworld')]) {
        try {
            const animatronics = dim.getEntities({ families: ['animatronic'] });
            for (const entity of animatronics) {
                try {
                    const loc = entity.location;
                    const entityId = entity.id;
                    
                    let nearestDoorFrame = null;
                    let nearestDist = Infinity;
                    
                    for (let dx = -1.5; dx <= 1.5; dx++) {
                        for (let dz = -1.5; dz <= 1.5; dz++) {
                    ...
                    if (nearestDoorFrame) {
                        const direction = nearestDoorFrame.permutation.getState('minecraft:cardinal_direction');
                        const doorLoc = nearestDoorFrame.location;
                        ...
                        for (const p of players) {
                            ...
                            const d = Math.sqrt(
                                Math.pow(pLoc.x - loc.x, 2) + 
                                Math.pow(pLoc.z - loc.z, 2)
                            );
                        ...
                        if (closestPlayer) {
                            const pLoc = closestPlayer.location;
                            
                            const centerX = doorLoc.x + 0.5;
                            const centerZ = doorLoc.z + 0.5;
                            
                            const distToCenter = Math.sqrt(
                                Math.pow(loc.x - centerX, 0.5) + 
                                Math.pow(loc.z - centerZ, 0.5)
                            );
                            
                            const lastPos = animatronicLastPos.get(entityId);
                            const isStuck = lastPos && 
                                Math.abs(lastPos.x - loc.x) < 0.05 && 
                                Math.abs(lastPos.z - loc.z) < 0.05;
                            
                            const isCentered = animatronicCentered.get(entityId);
                            const currentTime = Date.now();
                            const lastTeleport = animatronicTeleportCooldown.get(entityId);
                            const canTeleport = !lastTeleport || (currentTime - lastTeleport) > 100;
                            
                            if (isStuck && canTeleport) {
                                if (!isCentered && distToCenter > 0.3) {
                                    entity.teleport(
                                        { x: centerX, y: loc.y, z: centerZ }
                                    );
                                    animatronicCentered.set(entityId, true);
                                    animatronicTeleportCooldown.set(entityId, currentTime);
                                } else {
                                    ...
                                    if (Math.abs(dx) > Math.abs(dz)) {
                                        ...
                                        playerDirection = dz > 0 ? 'south' : 'north';
                                    }
                                    ...
                                    entity.teleport(
                                        { x: loc.x + moveX, y: loc.y, z: loc.z + moveZ },
                                        { rotation: { x: 0, y: rotY } }
                                    );
                                    animatronicTeleportCooldown.set(entityId, currentTime);
                                }
                            ...
                            animatronicLastPos.set(entityId, { x: loc.x, z: loc.z });
                        }
                    } else {
                        animatronicLastPos.delete(entityId);
                        animatronicCentered.delete(entityId);
                        animatronicTeleportCooldown.delete(entityId);
                    }
                } catch {}
            }
        } catch {}
