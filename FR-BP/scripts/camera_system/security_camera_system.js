...
class SecurityCameraSystem {
    constructor() {
        ...
        this.AUTO_PAN_SPEED = 1.25;
        this.AUTO_PAN_RANGE = 85;
        this.AUTO_PAN_DWELL_TICKS = 80;
        this.blockUpdateCounter = new Map();
        this.BLOCK_UPDATE_INTERVAL = 20;
        ...
        this.cameraEditMode = new Map();
        this.rangePreviewDebounce = new Map();
        this.rangePreviewAnim = new Map();
        this.RANGE_PREVIEW_DEBOUNCE_TICKS = 40;
        this.RANGE_PREVIEW_ANIM_TICKS = 60;
    ...
    _clearPlayerCamera(player) {
        ...
        try {
            player.setDynamicProperty("fr:viewing_camera_pos", undefined);
        } catch { }
        try {
            player.setDynamicProperty("fr:viewing_camera", undefined);
        } catch { }
        try {
            player.setDynamicProperty("fr:force_camera_switch", undefined);
        } catch { }
        try {
    ...
    _resetViewState(playerId) {
        ...
        this.autoPan.set(playerId, {
            offset: 0, dir: 1, dwell: 0,
        });
    ...
    onBlockInteract(event) {
        ...
        if (blockId === "fr:security_cameras") {
            if (itemId === "fr:faz-diver_security") {
                console.warn("[Debug] Selecting camera...");
        ...
        if (itemId === "fr:faz-diver_security") {
            ...
            if (sel && sel.type === "camera") {
                const rpData = getRoutePointData(block.location, player.dimension.id);
                if (rpData) {
                    const cameraPosStr = this.locStr(sel.pos);
                    updateRoutePoint(block.location, player.dimension.id, {
                        linkedCamera: cameraPosStr
                    }
                    );
                    player.sendMessage(
                        dynamicToast(
                            "§l§aLINKED TO POINT",
                            `§6Camera: §f${cameraPosStr} \n§6Route Point: §f#${rpData.order + 1} `,
    ...
    selectCamera(player, block) {
        ...
        const camPosStr = this.locStr(pos);
        const links = [];
        for (const [pcPosStr, camList] of this.connections.entries()) {
            if (camList.includes(camPosStr)) {
                links.push(`- Old PC [${pcPosStr.replace(/, /g, ",")}]`);
            }
        }
        for (const rp of routePointRegistry.values()) {
            if (rp.linkedCamera === camPosStr) {
                links.push(
                    `- Route Point #${rp.order + 1} [${Math.floor(rp.location.x)}, ${Math.floor(rp.location.y)}, ${Math.floor(rp.location.z)}]`,
                );
            }
        }
        const linkText =
            links.length > 0
                ...
                : "Current links:\n- None";
        console.warn(
            `[Debug] Sending message for cam ${camPosStr}. Links: ${links.length}`,
        );
        player.sendMessage(`[Debug] Links found: ${links.length}`);
        player.sendMessage(
            dynamicToast(
                "§l§aCAMERA SELECTED",
    ...
    showLockedPcUI(player, pcBlock) {
        try {
            const showLockedForm = () => {
                ...
                form.label("cr:sub_");
                form.label(`cr:txt_§0§lHead Director§r§0 - Polarfrederick §lAssistant Director§r§0 - Kristoffer1976 §lMain Developer§r§0 - Hyrxs §lConcept art§r§0 - Electro1987 - Guipcbonnie §lMusic/Sound Designer§r§0 - Werty_is_me - Foxlyticalxd §lAnimator§r§0 - M.forbidden. - elta_51 §lCoders§r§0 - Sgtsarnt3 - Warden45._31258 §lTexture designer§r§0 - Mansam47 - Kilokegor - Guipcbonnie - Foxisgaming_ - Firecaptain221 - Polarfrederick - Hyrxs - Kristoffer1976 - tin__fly - fantasticbob_mc §lBlock Modeler§r§0 - Mansam47 - Kilokegor - Guipcbonnie - Foxisgaming_ - Firecaptain221 - Polarfrederick - Hyrxs - Kristoffer1976 `);
    ...
    applyView(
        ...
        const finish = () => {
            ...
            try {
                player.setDynamicProperty("fr:viewing_camera_pos", camPosStr);
            } catch { }
            try {
                player.setDynamicProperty("fr:viewing_camera", true);
            } catch { }
            ...
            if (overlayMode && overlayMode.type === "edit") {
                this.applyCameraEditOverlay(
                    player,
                    pcPosStr,
                    camPosStr,
                    dimension,
                ...
                this.cameraEditMode.delete(pid);
                const debounceId = this.rangePreviewDebounce.get(pid);
                if (debounceId !== undefined) {
                    try { system.clearRun(debounceId); } catch { }
                }
                this.rangePreviewDebounce.delete(pid);
                const animId = this.rangePreviewAnim.get(pid);
                if (animId !== undefined) {
                    try { system.clearRun(animId); } catch { }
                }
                this.rangePreviewAnim.delete(pid);
    ...
    tickExitByCrouch() {
        try {
            ...
            const shouldUpdateHud = this.hudUpdateCounter >= 80;
            ...
            for (const pid of this.viewers) {
                ...
                if (!player) continue;
                try {
                    if (player.getDynamicProperty("fr:force_camera_switch")) {
                        player.setDynamicProperty("fr:force_camera_switch", undefined);
                ...
                if (shouldUpdateHud) {
                    let titleFlag = "§e§n§e§r§g§y§p";
                    if (session) {
                        const genPos = this.getGeneratorForPc(session.pc);
                        if (genPos) {
                            const dim = player.dimension;
                            const gen = getGeneratorAt({
                                ...genPos,
                                dimensionId: dim.id
                            });
                            if (gen && gen.energy <= 0) {
                    ...
                    try {
                        player.runCommand(`title @s title ${titleFlag} §r`);
                    } catch { }
                }
                if (player.isSneaking) {
                    this.exitView(player);
                ...
                try {
                    ...
                    const editMode = this.cameraEditMode.get(pid);
                    if (editMode === "pitch" || editMode === "fov" || editMode === "range") {
    ...
    exitViewToPc(player, pcPosStr) {
        try {
            const pid = player.id;
            if (this.pendingExit.has(pid)) return false;
            this.pendingExit.add(pid);
            this.viewers.delete(pid);
            ...
            try {
                this.pendingExit.delete(player.id);
            } catch { }
            return false;
        }
    }
    ...
    pcApproachPoseOf(block) {
        try {
            ...
            try {
                ...
                else if (face === "south") dir = {
                    x: 0, z: 1
                };
                else if (face === "east") dir = {
                    x: 1, z: 0
                };
                else if (face === "west") dir = {
                    x: -1, z: 0
                };
            } catch { }
            const distance = 1.2;
    ...
    applyControlOverlay(player, pcPosStr, camPosStr, dimension) {
        try {
            ...
            const genPosForCheck = this.getGeneratorForPc(pcPosStr);
            if (genPosForCheck) {
                const genForCheck = getGeneratorAt({ ...genPosForCheck, dimensionId: dimension.id });
                if (genForCheck && genForCheck.energy <= 0) {
                    isPowerOut = true;
                }
            }
    ...
    applyCameraEditOverlay(player, pcPosStr, camPosStr, dimension, settingType, state = null) {
        try {
            const pid = player.id;
            if (!this.viewers.has(pid)) return;

            this.cameraEditMode.set(pid, settingType);
            this.viewYaw.set(pid, 0);
            this.autoPan.set(pid, { offset: 0, dir: 1, dwell: 0 });
            ...
            if (!state) {
                const baseSettings = this._getCameraSettings(camPosStr);
                state = {
            ...
            const bounds = {
                pitch: { min: -90, max: 90, step: 5 },
                range: { min: 10, max: 180, step: 5 },
                fov: { min: 30, max: 110, step: 5 },
            };

            const getDraftValue = () => {
                if (settingType === "pitch") return state.draft.verticalPitch ?? 0;
                if (settingType === "range") return state.draft.rotationRange ?? defaults.range;
                return state.draft.fov ?? defaults.fov;
            };
            const setDraftValue = (val) => {
                if (settingType === "pitch") state.draft.verticalPitch = val;
                else if (settingType === "range") state.draft.rotationRange = val;
                else state.draft.fov = val;
            };
            const clamp = (v, min, max) => Math.max(min, Math.min(max, v));

            const render = () => {
                const b = bounds[settingType] ?? bounds.pitch;
                const current = getDraftValue();
                ...
                const modeFlag =
                    settingType === "pitch"
                        ...
                        : settingType === "range"
                            ? "§C§A§M§E§R"
                            : "§C§A§M§E§F";
                const title = `§r§r§r§C§u§C§A§M§E§D${modeFlag}§f${camName}`;
                ...
                const desc =
                    settingType === "pitch"
                        ? "Define the value the camera will look at"
                        : settingType === "range"
                            ? "Defines the range at which the camera will rotate sideways"
                            : "Defines the field of view (zoom) of the camera";

                const resetPrompt = "Want to revert to a default value?\nclick here";

                const leftGlyph =
                    settingType === "pitch" ? "§8" : settingType === "range" ? "§8<" : "§8-";
                const rightGlyph =
                    settingType === "pitch" ? "§8" : settingType === "range" ? "§8>" : "§8+";
                const valueText =
                    settingType === "pitch" ? `${Math.round(current)}°` : `${Math.round(current)}°`;

                form.button(header);
                form.button(desc);
                form.button(leftGlyph);
                form.button(valueText);
                form.button(rightGlyph);
                form.button(resetPrompt);
                form.button("Reset");
                form.button("Save");
                form.button(" ");
                ...
                form.show(player).then((res) => {
                    if (!this.viewers.has(pid)) return;
                    if (res.canceled || res.selection === undefined || res.selection === 8) {
                        const reverted = { ...state.original };
                        this.cameraSettings.set(camPosStr, reverted);
                        this._applyCameraSettingsPreview(player, dimension, camPosStr);
                        this.exitViewToPc(player, pcPosStr);
                        system.runTimeout(() => {
                            try {
                    ...
                    if (sel === 2 || sel === 4) {
                        const dir = sel === 2 ? -1 : 1;
                        const next = clamp(current + dir * b.step, b.min, b.max);
                        setDraftValue(next);
                        this.cameraSettings.set(camPosStr, { ...state.draft });
                        this._applyCameraSettingsPreview(player, dimension, camPosStr);
                        if (settingType === "range") {
                            this._scheduleRotationRangePreview(player, dimension, camPosStr);
                        }
                        system.run(() => this.applyCameraEditOverlay(player, pcPosStr, camPosStr, dimension, settingType, state));
                        return;
                    }
                    if (sel === 6) {
                        if (settingType === "pitch") setDraftValue(defaults.pitch);
                        else if (settingType === "range") setDraftValue(defaults.range);
                        else setDraftValue(defaults.fov);
                        this.cameraSettings.set(camPosStr, { ...state.draft });
                        this._applyCameraSettingsPreview(player, dimension, camPosStr);
                        if (settingType === "range") {
                            this._scheduleRotationRangePreview(player, dimension, camPosStr);
    ...
    _applyCameraSettingsPreview(player, dimension, camPosStr) {
        try {
            const pid = player.id;
            const camBlock = this.blockFromLocStr(dimension, camPosStr);
            if (!camBlock || camBlock.typeId !== "fr:security_cameras") return;
            const pose = this.frontPoseOf(camBlock);
            const yawOffset = this.viewYaw.get(pid) ?? 0;
            const yaw = pose.yaw + yawOffset;
            const qYaw = Math.round(yaw / 2) * 2;
            const ok = this.trySetCamera(player, pose.pos, qYaw, -pose.pitch);
            if (!ok) return;
            this.baseYaw.set(pid, pose.yaw);
            this.lastAppliedYaw.set(pid, qYaw);
            const settings = this._getCameraSettings(camPosStr);
            const fov = settings.fov ?? 70;
            try {
    ...
    _scheduleRotationRangePreview(player, dimension, camPosStr) {
        try {
            const pid = player.id;
            if (!this.viewers.has(pid)) return;
            if (this.cameraEditMode.get(pid) !== "range") return;
            const oldDebounce = this.rangePreviewDebounce.get(pid);
            if (oldDebounce !== undefined) {
                try { system.clearRun(oldDebounce); } catch { }
            }
            this.rangePreviewDebounce.delete(pid);
            const oldAnim = this.rangePreviewAnim.get(pid);
            if (oldAnim !== undefined) {
                try { system.clearRun(oldAnim); } catch { }
            }
            this.rangePreviewAnim.delete(pid);
            const id = system.runTimeout(() => {
                this.rangePreviewDebounce.delete(pid);
                this._startRotationRangePreview(player, dimension, camPosStr);
            }, this.RANGE_PREVIEW_DEBOUNCE_TICKS);
            this.rangePreviewDebounce.set(pid, id);
        } catch { }
    }

    _startRotationRangePreview(player, dimension, camPosStr) {
        try {
            const pid = player.id;
            if (!this.viewers.has(pid)) return;
            if (this.cameraEditMode.get(pid) !== "range") return;
            ...
            const settings = this._getCameraSettings(camPosStr);
            const pitch = settings.verticalPitch ?? 0;
            const half = (settings.rotationRange ?? this.AUTO_PAN_RANGE) * 0.5;
            const camLoc = camBlock.location;
            const centerX = camLoc.x + 0.5;
            const centerY = camLoc.y + 0.6;
            const centerZ = camLoc.z + 0.5;
            ...
            let t = 0;
            const total = this.RANGE_PREVIEW_ANIM_TICKS;
            const seg = Math.max(1, Math.floor(total / 3));
            const intervalId = system.runInterval(() => {
                try {
                    if (!this.viewers.has(pid) || this.cameraEditMode.get(pid) !== "range") {
                    ...
                    if (t <= seg) {
                        ...
                        offset = half + ((t - seg) / seg) * (-2 * half);
                    } else {
                        offset = -half + ((t - seg * 2) / (total - seg * 2 || 1)) * half;
                    }
                    const yaw = baseYawValue + offset;
                    const yawRad = yaw * this.DEG_TO_RAD;
                    const sin = Math.sin(yawRad);
                    const cos = Math.cos(yawRad);
                    const pos = {
                        x: centerX - sin * offsetDist,
                        y: centerY,
                        z: centerZ + cos * offsetDist,
                    };
                    const qYaw = Math.round(yaw / 2) * 2;
                    this.trySetCamera(player, pos, qYaw, -pitch);
                    t += 1;
                } catch {
            ...
            this.rangePreviewAnim.set(pid, intervalId);
        } catch { }
    }
            ...
            form
                ...
                .then((res) => {
                    ...
                    if (selection === 0) {
                        ...
                        this.toggleCameraDisabled(player, pcPosStr, camPosStr);
                    } else if (selection === 3) {
                        this.applyView(
                            player,
                            player.dimension,
    ...
    toggleCameraDisabled(player, pcPosStr, camPosStr) {
        try {
            const settings = this._getCameraSettings(camPosStr);
            settings.disabled = !settings.disabled;
            this.cameraSettings.set(camPosStr, settings);
            this.saveCameraSettings();
            const statusText = settings.disabled ? "Disabled" : "Enabled";
            const icon = settings.disabled ? "textures/fr_ui/deny_icon" : "textures/fr_ui/approve_icon";
            const bg = settings.disabled ? "textures/fr_ui/deny_ui" : "textures/fr_ui/approve_ui";
            player.sendMessage(
                dynamicToast(
                    settings.disabled ? "§l§cCAMERA DISABLED" : "§l§qCAMERA ENABLED",
                    settings.disabled
                        ? "§7Camera feed disabled - Audio only mode"
                        : "§7Camera feed enabled",
                    icon,
                    bg));
            system.runTimeout(() => {
                this.editCamera(player, pcPosStr, camPosStr);
    ...
    showChangelog(player, pcBlock) {
        try {
            const form = new ActionFormData()
                ...
                .label(`§7- The door frame no longer exists - Hand and spear animation - Disappearance of doors over long distances - The vanilla menu appeared behind the camera settings - Sometimes the flashlight wouldn't turn off when leaving the PC or simply when it rotated too much - The wall blocks did not have a different top and bottom face - Some texts in some items did not have a translation - The animatronics couldn't pass through the door frame - Now, when you select a door block, it gives you the door item and no longer a glitched door block`);
            form
                .show(player)
                .then((res) => {
                    this.showPcMainMenu(player, pcBlock, true);
...
export {
    securityCameraSystem, isPlayerInCamera
};
