...
system.beforeEvents.startup.subscribe(({ customCommandRegistry }) => {
  ...
  customCommandRegistry.registerEnum("fr:uiType", [
    "newspaper",
    "start-screen",
    "custom-black-screen",
    "custom-toast",
  ...
  customCommandRegistry.registerEnum("fr:generatorAction", [
    "energy",
    "limit",
    "drain",
  ]);
  customCommandRegistry.registerEnum("fr:connectionType", [
    "switch",
    "door_button",
  ]);
  customCommandRegistry.registerEnum("fr:fazTabRangeType", ["max", "min"]);
  customCommandRegistry.registerEnum("fr:routeAction", [
    "delete",
    "count",
    "deleteall",
    "clear_cam_effects",
    ...
    (origin, rangeType, value) => {
      if (rangeType === "max") {
        ...
        return {
          ...
          message: `Faz-Tab max range set to ${value} blocks`,
        };
      }
      if (rangeType === "min") {
        ...
        return {
          status: CustomCommandStatus.Success,
          message: `Faz-Tab min range set to ${value} blocks`,
        };
      }
      return {
        status: CustomCommandStatus.Failure,
        message: "Unknown range type",
      };
    },
  );
    ...
    (origin, action, x, y, z, value) => {
      if (action === "energy") {
        if (
          ...
          return {
            status: CustomCommandStatus.Failure,
            message: "Usage: /fr:generator energy <x> <y> <z> <value>",
          };
        }
        const dimensionId =
          origin.sourceEntity?.dimension?.id || "minecraft:overworld";
      ...
      if (action === "limit") {
        if (x === undefined) {
          return {
            status: CustomCommandStatus.Failure,
            message: "Usage: /fr:generator limit <value>",
          };
        }
        setGeneratorLimit(x);
        return {
          status: CustomCommandStatus.Success,
          message: `Generator connection limit set to ${x}`,
        };
      }
      if (action === "drain") {
        if (
          ...
          return {
            status: CustomCommandStatus.Failure,
            message:
              "Usage: /fr:generator drain <x> <y> <z> <percentage 10-200>",
          };
        }
        const dimensionId =
          origin.sourceEntity?.dimension?.id || "minecraft:overworld";
  ...
  customCommandRegistry.registerCommand(
    ...
    (origin, connectionType, value) => {
      if (connectionType === "switch") {
        ...
        return {
          status: CustomCommandStatus.Success,
          message: `Switch connection limit set to ${value}`,
        };
      }
      if (connectionType === "door_button") {
        ...
        return {
          status: CustomCommandStatus.Success,
          message: `Door Button connection limit set to ${value}`,
        };
      }
      return {
        status: CustomCommandStatus.Failure,
  ...
  customCommandRegistry.registerCommand(
    ...
    (origin, hudType, targets, state, x, y, z) => {
      ...
      if (hudType === "clock") {
        ...
        return {
          status: CustomCommandStatus.Success,
          message: `${showHud ? "Showing" : "Hiding"} clock HUD for ${targets.length} player(s)`,
        };
      }
      ...
      if (hudType === "generator") {
        if (
          ...
          return {
            status: CustomCommandStatus.Failure,
            message: "Usage: /fr:hud generator <player> true <x> <y> <z>",
          };
        }
        system.run(() => {
          for (const player of targets) {
        ...
        return {
          status: CustomCommandStatus.Success,
          message: `${showHud ? "Showing" : "Hiding"} generator HUD for ${targets.length} player(s)`,
        };
      }

      return {
        status: CustomCommandStatus.Failure,
        message: "Unknown HUD type",
      };
    },
  );
  ...
  customCommandRegistry.registerCommand(
    ...
    (origin, targets, featureType, uiType, time, ampm, nightNumber) => {
      ...
      if (featureType === "UI" && uiType === "newspaper") {
        ...
        return {
          status: CustomCommandStatus.Success,
          message: `Showing newspaper to ${targets.length} player(s)`,
        };
      }
      ...
      if (featureType === "UI" && uiType === "start-screen") {
          ...
          for (const player of targets) {
            ...
            const showStartScreen = (timeText, nightText) => {
              system.runTimeout(() => {
                ...
                form.body("");
                form.button(`${timeText}`);
                form.button(`${nightText}`);
            ...
            if (useManual) {
              ...
              securityCameraSystem
                .getWorldTimeLabelsAsync(player)
                .then((labels) => {
                  ...
                  const autoTimeText = hour
                    ? `${hour}:00 ${ampmLabel}`.trim()
                    : clockRaw;
        ...
        return {
          status: CustomCommandStatus.Success,
          message: `Showing start screen to ${targets.length} player(s)`,
        };
      }
      ...
      if (featureType === "UI" && uiType === "custom-black-screen") {
        system.run(() => {
          for (const player of targets) {
            ...
            system.runTimeout(() => {
              ...
              if (line1) form.button(line1);
              if (line2) form.button(line2);
        ...
        return {
          status: CustomCommandStatus.Success,
          message: `Showing custom black screen to ${targets.length} player(s)`,
        };
      }
      ...
      if (featureType === "UI" && uiType === "custom-toast") {
        ...
        return {
          status: CustomCommandStatus.Success,
          message: `Showing custom toast to ${targets.length} player(s)`,
        };
      }
  ...
  customCommandRegistry.registerCommand(
    {
      name: "fr:route",
      description: "Manage animatronic route points",
      permissionLevel: CommandPermissionLevel.GameDirectors,
      mandatoryParameters: [
        { name: "fr:routeAction", type: CustomCommandParamType.Enum },
      ],
      optionalParameters: [
        { name: "routeId", type: CustomCommandParamType.Integer },
        { name: "point", type: CustomCommandParamType.Integer },
      ],
    },
    (origin, action, routeId, point) => {
      if (action === "clear_cam_effects") {
        if (routeId === undefined || point === undefined) {
          return {
            status: CustomCommandStatus.Failure,
            message: "Usage: /fr:route clear_cam_effects <routeId> <pointNumber>",
          };
        }
        ...
        if (routePoints.length === 0) {
          return {
            status: CustomCommandStatus.Failure,
            message: `No route points found for route ID ${routeId}`,
          };
        }
        const idx = Math.floor(point) - 1;
        if (idx < 0 || idx >= routePoints.length) {
          return {
            status: CustomCommandStatus.Failure,
            message: `Invalid point number. Route ${routeId} has ${routePoints.length} point(s).`,
          };
        }
        const rp = routePoints[idx];
        const effects = Array.isArray(rp.effects) ? rp.effects : [];
        const filtered = effects.filter(
          (e) =>
            e?.type !== EFFECT_TYPES.CAMERA_FORCE_SWITCH &&
            e?.type !== EFFECT_TYPES.SCREEN_FADE &&
            e?.type !== EFFECT_TYPES.CAMERA_BLACKOUT,
        );

        updateRoutePoint(rp.location, rp.dimensionId, {
          linkedCamera: null,
          effects: filtered,
        });
        return {
          status: CustomCommandStatus.Success,
          message: `Cleared camera effects from route ${routeId} point #${idx + 1}`,
        };
      }
      if (action === "delete") {
        if (routeId === undefined) {
          return {
            status: CustomCommandStatus.Failure,
            message: "Usage: /fr:route delete <routeId>",
          };
        }
        const routePoints = getRoutePointsForRouteId(routeId);
        if (routePoints.length === 0) {
        ...
        deletedCount = deleteAllRoutePoints(routeId);

        return {
          status: CustomCommandStatus.Success,
          message: `Deleted ${deletedCount} route points from route ${routeId}`,
        };
      }
      ...
      if (action === "count") {
        if (routeId === undefined) {
          return {
            status: CustomCommandStatus.Failure,
            message: "Usage: /fr:route count <routeId>",
          };
        }
        const routePoints = getRoutePointsForRouteId(routeId);
        return {
          status: CustomCommandStatus.Success,
          message: `Route ${routeId} has ${routePoints.length} route points`,
        };
      }
      ...
      if (action === "deleteall") {
        ...
        const allKeys = world
          ...
          .filter((id) => id.startsWith("fr:rp_"));
          ...
          try {
            ...
            if (data) {
              ...
              try {
                ...
                const block = dimension.getBlock(parsed.location);
                if (block && block.typeId === "fr:route_point") {
                  block.setType("minecraft:air");
                }
              } catch (e) { }
        ...
        return {
          status: CustomCommandStatus.Success,
          message: `Deleted ${totalDeleted} route points from all routes`,
        };
      }

      return {
        status: CustomCommandStatus.Failure,
        message: "Unknown action. Use: delete, count, or deleteall",
      };
    },
  );
});
